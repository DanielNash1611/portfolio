...
export default async function SoundSeekerPage(): Promise<JSX.Element> {
  ...
  const heroMetrics = [
    ...
    },
    {
      ...
      value: "87%",
      description:
        "Musicians in usability testing who said they would use Sound Seeker again.",
      tone: "dark",
  ...
  const outcomeCards = [
    {
      ...
      description:
        "Judged by the C-suite, positioning Sound Seeker as a flagship AI concept.",
    },
    {
    ...
    <div className="container space-y-10 py-10">
      ...
      <div className="rounded-3xl border border-brand-slate/10 bg-gradient-to-br from-brand-cream/80 to-brand-tan/60 p-10 shadow-soft">
        <div className="space-y-4">
          ...
          <p className="text-lg text-brand-slate/80">
            An AI assistant that helps musicians jump from &ldquo;I love that
            sound&rdquo; to a concrete rig: gear, signal chain, and starter
            settings.
          </p>
          <div className="flex flex-wrap items-center gap-4">
            <a
              href="#sound-seeker-demo"
              className="inline-flex items-center rounded-full bg-brand-teal px-6 py-2.5 text-sm font-semibold text-brand-cream shadow-sm"
            >
              Open Demo
            </a>
            <p className="text-sm text-brand-slate/70">
              Working text-based demo embedded below. Rebuilt independently
              using public data and OpenAI APIs; no proprietary assets.
            </p>
          </div>
        </div>
      </div>
      ...
      <article className="space-y-10 rounded-3xl border border-brand-slate/10 bg-white/90 p-6 shadow-soft md:p-10">
        <div className="rounded-2xl border border-brand-slate/20 bg-white/80 p-4 text-sm text-brand-slate/80">
          <p>
            <strong>Note:</strong> This project was originally conceived during
            a Guitar Center hackathon under a different name.
          </p>
          <p className="mt-1">
            This portfolio version uses the name{" "}
            <span className="font-semibold text-brand-teal">Sound Seeker</span>{" "}
            to avoid confusion with Guitar Center&rsquo;s internal prototype
            naming.
          </p>
        </div>
        <section className="rounded-3xl bg-gradient-to-br from-[#DBBF96] via-[#F2E3D5] to-[#D17A5F] p-6 text-[#2C4F52] shadow-lg md:p-10">
          <div className="grid items-center gap-8 md:grid-cols-[2fr,1.5fr]">
            <div className="space-y-4">
              ...
              <h2 className="text-3xl font-semibold md:text-4xl">
                Sound Seeker AI Gear Recommender for Musicians
              </h2>
              <p className="text-sm text-[#3A3D40] md:text-base">
                An AI assistant that helps musicians jump from &ldquo;I love
                that sound&rdquo; to a concrete rig: gear, signal chain, and
                starter settings. Born in Guitar Center&rsquo;s first ChatGPT
                Hackathon, later rebuilt as an independent demo using public
                data and OpenAI APIs.
              </p>
              <div className="flex flex-wrap gap-2 pt-2">
                {heroChips.map((chip) => (
                  <span
                    key={chip}
                    className="inline-flex items-center rounded-full bg-[#2C4F52] px-3 py-1 text-xs font-semibold text-[#F2E3D5]"
                  >
                    {chip}
                  </span>
                ))}
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-white/40 pt-4 text-xs text-[#3A3D40] md:text-sm">
            <a
              href="#sound-seeker-demo"
              className="rounded-full bg-[#2C4F52] px-4 py-2 font-semibold text-[#F2E3D5] shadow-sm"
            >
              Open Demo
            </a>
            <p>
        ...
        <section className="grid gap-4 md:grid-cols-3">
          {outcomeCards.map((card) => (
            ...
            >
              ...
              <p className="mt-2 text-sm font-semibold text-[#2C4F52]">
                {card.value}
              </p>
        ...
        <section className="grid gap-10 md:grid-cols-2">
          <div>
            <h3 className="mb-2 text-base font-semibold text-[#2C4F52]">
              The Challenge
            </h3>
            <p className="text-sm text-slate-800">
              Musicians burn hours chasing tones across videos and forum
              threads. We wanted a faster path from &ldquo;I love that
              sound&rdquo; to &ldquo;here is how to set up my rig&rdquo; â€”
              without handing creativity to a black box.
            </p>
          </div>
          <div>
            <h3 className="mb-2 text-base font-semibold text-[#2C4F52]">
        ...
        <section className="rounded-3xl bg-[#F2E3D5] p-6 md:p-8">
          <h3 className="mb-4 text-base font-semibold text-[#2C4F52] md:text-lg">
            How Sound Seeker Works
          </h3>
        ...
        <section className="grid gap-10 md:grid-cols-2">
          <div>
            <h3 className="mb-3 text-base font-semibold text-[#2C4F52]">
              Technology
            </h3>
        ...
        <section className="rounded-3xl bg-white p-6 shadow-sm">
          ...
          <p className="text-sm text-slate-800">
            Sound Seeker went from a deprioritized hackathon idea to a 1st-place
            prototype that helped shift Guitar Center&rsquo;s AI strategy. The
            independent demo on this page shows how curiosity plus AI tools let
            a Product Manager ship meaningful, musician-facing prototypes fast.
          </p>
        </section>
      </article>

      <section
        id="sound-seeker-demo"
        className="rounded-3xl bg-[#2C4F52] p-6 text-[#F2E3D5] shadow-soft md:p-8"
      >
        <h2 className="text-base font-semibold md:text-lg">
          Try the Sound Seeker prototype
        </h2>
        <p className="mt-2 text-sm text-[#F2E3D5]/80">
          This interactive demo taps GPT-5 with grounded web search to recommend
          gear chains, plugin setups, and production tweaks tailored to the tone
