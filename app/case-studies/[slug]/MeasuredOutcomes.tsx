      ...
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        {items.map((m, idx) => (
          ...
          >
            ...
            {m.context && (
              <p className="mt-1 text-sm text-slate-600">{m.context}</p>
            )}
          </div>
        ))}
      </div>
      <p className="mt-3 text-xs text-slate-500">
        Metrics shown are measured results tied to this case study; broader
        portfolio impact appears on the home page.
      </p>
    </section>
  );
}
