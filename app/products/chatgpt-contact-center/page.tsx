...
import { caseStudySocialProof } from "@/data/socialProof";
    ...
    <div className="container space-y-12 py-10">
      ...
      <section className="space-y-4 rounded-3xl border border-brand-slate/10 bg-white/90 p-8 shadow-soft">
        ...
        <dl className="grid gap-4 md:grid-cols-2">
          {snapshotItems.map((item) => (
            <div
              key={item.label}
              className="rounded-2xl border border-brand-slate/10 bg-white/80 p-4"
            >
              <dt className="text-sm font-semibold uppercase tracking-[0.2em] text-brand-teal/70">
                {item.label}
              </dt>
              <dd className="mt-2 text-base text-brand-slate/80">
                {item.value}
              </dd>
            </div>
          ))}
      ...
      <section className="space-y-4 rounded-3xl border border-brand-slate/10 bg-white/90 p-8 shadow-soft">
        ...
        <p className="text-brand-slate/80">
          Agents juggled multiple tools, manually drafted emails and call
          scripts, and spent valuable time preparing for every interaction.
          Leadership wanted to know if AI could serve as a safe, reliable
          copilot that improved performance without risking customer trust.
        </p>
        <ul className="list-disc space-y-2 pl-6 text-brand-slate/80">
          <li>
            Help agents prepare before calls with faster access to relevant
            context.
          </li>
          <li>
            Draft on-brand emails in minutes instead of grinding through
            templates and copy/paste loops.
          </li>
          <li>
            Deliver measurable lift in Revenue Per Call, Items Per Transaction,
            and Average Order Value.
          </li>
          <li>
            Respect Guitar Center&apos;s brand voice and data governance
            requirements.
          </li>
        </ul>
      </section>
      ...
      <section className="space-y-4 rounded-3xl border border-brand-slate/10 bg-white/90 p-8 shadow-soft">
        ...
        <ul className="list-disc space-y-2 pl-6 text-brand-slate/80">
          <li>
            Framed the opportunity, success metrics, and business case for
            piloting AI inside the contact center.
          </li>
          <li>
            Partnered with EIS and Legal to ensure security, privacy, and
            compliance were baked into the pilot from day one.
          </li>
          <li>
            Facilitated co-design sessions with supervisors and agents to
            capture real workflows and pain points.
          </li>
          <li>
            Defined and iterated on customGPT instructions, tone guidance, and
            workflow prompts.
          </li>
          <li>
            Designed the experimental setup, including control group selection
            and measurement methodology.
          </li>
          <li>
            Socialized pilot results and recommendations with senior leadership
            to green-light expansion.
          </li>
        </ul>
      </section>

      <section className="space-y-4 rounded-3xl border border-brand-teal/12 bg-brand-teal/5 p-8 shadow-soft">
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-[0.28em] text-brand-teal/70">
            External validation
          </p>
          <h2 className="text-2xl font-semibold text-brand-teal md:text-3xl">
            How collaborators described the same work
          </h2>
        </div>
      ...
      <section className="space-y-6 rounded-3xl border border-brand-slate/10 bg-white/90 p-8 shadow-soft">
        <div className="space-y-3">
          ...
          <p className="text-brand-slate/80">
            The pilot focused on shipping working workflows quickly - balancing
            agent creativity with the guardrails needed for a retail contact
            center.
          </p>
        </div>
        <div className="space-y-6">
          {approachSections.map((section) => (
            <div
              key={section.title}
              className="space-y-3 rounded-2xl border border-brand-teal/10 bg-brand-teal/5 p-5"
            >
      ...
      <section className="space-y-4 rounded-3xl border border-brand-slate/10 bg-white/90 p-8 shadow-soft">
        ...
        <p className="text-brand-slate/80">
          Retail seasonality and promotional spikes make simple before/after
          comparisons unreliable. We designed a matched control methodology to
      ...
      <section className="space-y-4 rounded-3xl border border-brand-slate/10 bg-white/90 p-8 shadow-soft">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {outcomes.map((outcome) => (
            <div
              key={outcome.heading}
              className="space-y-2 rounded-2xl border border-brand-teal/10 bg-white p-5 shadow-sm"
            >
      ...
      <section className="rounded-3xl border border-brand-teal/15 bg-brand-teal/10 p-8 text-brand-teal shadow-soft">
        <div className="space-y-3">
          ...
          <p className="text-brand-teal/80">
            Let&apos;s talk about your AI roadmap and how to launch or scale
            copilots responsibly.
          </p>
          <Link href="/contact">
