...
// engineClient.ts bypasses this entirely. This mock is only reachable when
// RESUME_GENERATOR_ENABLE_MOCK=true in a non-production environment.
//
// Behavior:
...
import { RESUME_MOCK_LABEL, type ResumeJobStatus } from "./types";
import type {
...
const STORE_KEY = "__portfolioResumeMockEngineStore";

function getStore(): MockEngineStore {
  const globalStore = globalThis as typeof globalThis & {
    [STORE_KEY]?: MockEngineStore;
  };

  globalStore[STORE_KEY] ??= {
    jobs: new Map<string, MockJob>(),
    byClientRequestId: new Map<string, string>(),
  };
  ...
  if (fraction >= 1 && !job.shouldFail) {
    ...
    return {
      ...
      result: {
        ...
        fitSummary: `${RESUME_MOCK_LABEL}. Placeholder generated by the in-memory mock engine — the production ResumeCustomizer engine renders the real tailored resume from Daniel's verified experience.`,
        mock: true,
...
export function mockCreateJob(
  ...
  const existingId = store.byClientRequestId.get(req.clientRequestId);
  if (existingId) {
  ...
  store.byClientRequestId.set(req.clientRequestId, jobId);
...
export function mockGetPdf(jobId: string): InternalPdfResult {
  ...
  // ASCII dash in the title: buildMockPdf renders StandardEncoding Helvetica, so
  // the canonical em-dash would be mojibake. The em-dash form stays in the
  // result metadata / UI / email where Unicode renders correctly.
  const bytes = buildMockPdf(RESUME_MOCK_LABEL.replace("—", "-"), [
